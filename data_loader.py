...
def get_shape_500m():
    field_500m = np.load(
        '/cnrm/recyf/Data/users/danjoul/dataset/data_train/G9KP_2021-01-01T00:00:00Z_rr.npy')
...
def get_shape_2km5():
    field_2km5 = np.load(
        '/cnrm/recyf/Data/users/danjoul/dataset/data_train/oper_c_2021-01-01T00:00:00Z_rr.npy')
...
def load_X_y(dates, echeances, data_location, params, resample='c'):
    ...
    for i_d, d in enumerate(dates):
        try:
            ...
            for i_p, p in enumerate(params):
                filepath_X = data_location + 'oper_c_' + d.isoformat() + 'Z_' + \
                    p + '.npy'
                X[i_d, :, :, :, i_p] = np.load(
    ...
    X = X[:, int(0.5 * (shape_2km5[0] - get_size_2km5()[1])):int(0.5 * (shape_2km5[0] + get_size_2km5()[1])), 
            int(0.5 * (shape_2km5[1] - get_size_2km5()[1])):int(0.5 * (shape_2km5[1] + get_size_2km5()[1])), :]
